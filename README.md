...
| `ConfineScanToContainingRoom` | `bool` | When true, TSDF skips voxels outside the MRUK room that contained the headset at scan start (conservative AABB, then inset clip planes). Default **false**. Set before `StartScanAsync`. `SCREEN` (TV) plane stamps still apply either way — as a dedicated voxel-AABB dispatch after Integrate, not a per-frustum search |
