...
- **Pre-Bake Mesh Simplification** — `meshopt_simplify` reduces the mesh before the UV unwrap so the atlas is registered to the displayed geometry; the dense mesh still drives occlusion during the bake. Keyframes are registered to the first-pass atlas (ZNCC shift → rotation) and blended with a per-chart preferred view so views agree instead of seaming.
...
3. Click **`Apply Game-Ready Setup`** at the top of the wizard. One click does the following — there's nothing else you need to configure first:
   ...
   - Adds the lean game-ready scene modules to a `RoomScan` root: `RoomScanner`, `RoomScanPersistence`, `RoomAnchorManager`, `PassthroughCameraProvider`, `TextureRefinement` (with 0.5 pre-bake simplification), `RoomUnderstanding`, `RoomScanSession`. Skips `TriplanarCache`, Gaussian Splat, and debug HUD to keep the build lean
...
RoomScans/
  ...
  pkg_20260228_143022/
    ...
    simplified_mesh.bin   # Legacy (pre-1.2): separate post-bake simplified copy; new bakes simplify before the bake
...
- **[meshoptimizer](https://github.com/zeux/meshoptimizer)** v1.0 by Arseny Kapoulkine (MIT) — mesh optimization toolkit. `meshopt_simplify` reduces the mesh before the UV unwrap and bake. Set `TextureRefinement.postBakeSimplificationRatio` < 1.0 to enable.
