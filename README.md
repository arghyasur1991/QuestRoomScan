...
- **Temporal Stabilization** — Adaptive per-vertex temporal blending on GPU prevents mesh jitter while allowing fast convergence. Optional live-mesh **birth fade** (inspector `birthFadeExtracts`) tints newly extracted verts cyan then settles to photoreal over N extracts so a low mesh-extract rate does not read as stop-motion. Opaque, no extra pass; refined atlas is unchanged.
