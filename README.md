...
- **Temporal Stabilization** — Adaptive per-vertex temporal blending on GPU prevents mesh jitter while allowing fast convergence. Optional live-mesh **birth fade** (`birthFadeSeconds` / `birthGrowMetres`) tints newly extracted verts cyan and grows them out along the normal **every frame** between mesh extracts. Optional **hold-and-morph** (`meshMorphSeconds`) lerps each dump from the previous voxel position and will not start the next extract until that lerp finishes (the TSDF keeps integrating). Opaque, no extra pass; refined atlas is unchanged.
