...
- **Temporal Stabilization** — Adaptive per-vertex temporal blending on GPU prevents mesh jitter while allowing fast convergence. Optional live-mesh **birth fade** (`birthFadeSeconds` / `birthGrowMetres`) tints newly extracted verts cyan and grows them out along the normal **every frame** between mesh extracts so a low extract rate does not read as stop-motion. Opaque, no extra pass; refined atlas is unchanged.
